...
const Index: React.FC = () => {
  ...
  const [actorType, setActorType] = useState<string[]>(filters.actorType ?? ["agent", "system"]);
                    ...
                    <Grid item xs={12} sm={6} md={3}>
                      <FormControl fullWidth disabled={!type}>
                        <Autocomplete
                          ...
                          renderInput={(params) => (
                            <TextField {...params} label="Action" placeholder={type ? "All" : "Select a type"} />
                          )}
                        ...
                        {logs.data.length === 0 ? (
                          ...
                          logs.data.map((row) => (
                            <TableRow key={row.id} hover>
                              <TableCell width={180}>{dayjs(row.created_at).format("LLL")}</TableCell>
                              <TableCell width={160}>
