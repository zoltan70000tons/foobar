...
const SplitPaymentModal: React.FC<SplitPaymentModalProps> = ({
  ...
  useEffect(() => {
    ...
    passengers.forEach((passenger) => {
      ...
      if (deletedPayments && deletedPayments.length > 0) {
        const deleted = deletedPayments.find((p) => p.passenger_id === passenger.id);
        if (deleted) {
  ...
  const handleBlur = (passengerId: number) => {
    setFormData((prevData: FormData): FormData => {
      ...
      let numericValue = parseFloat((prevData[key]?.amount || "").replace(/,/g, "")) || 0;
      ...
      passengers.forEach((p) => {
        ...
        const enteredAmount = parseFloat(String(newFormData[passengerKey]?.amount ?? "").replace(/,/g, "")) || 0;
            ...
            <Grid container spacing={2} mt={1} rowGap={2}>
              ...
              <Grid mt={0} ml={2} width={"100%"}>
                <Grid display={"flex"} width={"100%"}>
                  ...
                  <Grid>{formData?.totalLeftToPay?.toFixed(2)}</Grid>
                </Grid>
                <Grid display={"flex"} width={"100%"}>
                  <Grid xs={3} sx={{ fontWeight: 700 }}>
                    Total payment added:
                  </Grid>
                  <Grid>{formData?.totalPaymentAdded?.toFixed(2)}</Grid>
                </Grid>
              </Grid>
            </Grid>
