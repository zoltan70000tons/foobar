...
import {
  ...
  Box,
  Tooltip,
  IconButton, Typography, Stack, Button,
} from "@mui/material";
...
const View = ({ auth, potentialMatch, booking, event, otherPassenger }: Props) => {
  ...
  const handleEdit = () => {
    put(
      route('matches.update', { id: potentialMatch.id }),
      {
        onSuccess: () => {
  ...
  const handleDelete = () => {
    ...
    if (confirmed) {
      destroy(
        route('matches.destroy', { id: potentialMatch.id }),
        {
          onSuccess: () => {
  ...
  const handleResolve = () => {
    put(
      route('matches.resolve', { id: potentialMatch.id }),
      {
        onSuccess: () => {
