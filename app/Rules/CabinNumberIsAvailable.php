...
class CabinNumberIsAvailable implements ValidationRule {
    ...
    public function validate(string $attribute, mixed $value, Closure $fail): void {
        ...
        if ($exists) {
            $fail(
                'The :attribute is already booked in this event. Please choose a different number or remove the existing booking first.',
            );
        }

        $cabinNumberExistsInSpecs = DB::table('cabin_specs')->where('cabin_number', $value)->exists();
