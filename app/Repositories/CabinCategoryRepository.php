    ...
    function getCategoriesWithCabins($event_id) {
        $categories = CabinCategory::with([
            ...
            'cabins' => function ($query) {
                $query
                    ->select(['id', 'cabin_category_id', 'cabin_type_id', 'status'])
                    ->with('cabinType:id,cabin_type')
        ...
        // Prevent cabin appends from pulling cabin specs (would trigger N+1)
        $categories->each(function ($category) {
            $category->cabins->each->setAppends([]);
        });
