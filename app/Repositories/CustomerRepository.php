...
class CustomerRepository implements CustomerInterface {
    ...
    function update(CustomerRequest $request, User $user) {
        try {
            ...
            if (
                ...
                $request->input('middle_name') !== $originalDetailData['middle_name']
            ) {
            ...
            if ($request->filled('email') && $request->input('email') !== $originalUserData['email']) {
                $logUserDescription[] = "email was changed from {$originalUserData['email']} to {$request->input(
                    'email',
                )}";
            ...
            if (
                $request->filled('citizenship') &&
                $request->input('citizenship') !== $originalDetailData['citizenship']
            ) {
            ...
            if ($request->filled('phone') && $request->input('phone') !== $originalDetailData['phone']) {
                $logUserDescription[] = "phone was changed from {$originalDetailData['phone']} to {$request->input(
                    'phone',
                )}";
            ...
            if (
                $request->filled('dob') &&
                date('Y-m-d', strtotime($request->input('dob'))) !== $originalDetailData['dob']
            ) {
            ...
            if (
                ...
                $request->input('postal_code') !== $user->customerAddress->postal_code
            ) {
    ...
    function getPaginatedCustomerData($page, $perPage, $sortBy, $sortDir, $filters, $tags): LengthAwarePaginator {
        ...
        $baseQuery = DB::table('users as u')
            ...
            ->leftJoinSub($tagsAgg, 'tagg', function ($join) {
                $join->on(DB::raw('u.id::text'), '=', 'tagg.entity_id');
            });

        $filters = array_filter($filters ?? []);
        if (isset($filters['tags']) && $filters['tags'] !== '') {
            $term = $filters['tags'];
            $baseQuery->whereExists(function ($q) use ($term) {
                $q->select(DB::raw(1))
                    ->from('taggings as tg')
                    ->join('tags as t', 't.id', '=', 'tg.tag_id')
                    ->where('tg.entity_type', 'customer')
                    ->whereColumn('tg.entity_id', DB::raw('u.id::text'))
        ...
        $users->getCollection()->transform(function ($u) {
            $u->tags = json_decode($u->tags_json ?? '[]', true);
            unset($u->tags_json);
            return $u;
        });
