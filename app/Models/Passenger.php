...
class Passenger extends Authenticatable {
    ...
    public function getPaymentInfoAttribute() {
        try {
            ...
            $mergedDetails = $installments->map(function ($installment) use ($paymentAmount) {
                ...
                $lastPaymentDate = optional($installment->payments->sortByDesc('transaction_date')->first())
                    ->transaction_date;
     ...
     */
    public static function checkSurvivorInActiveBookings(
    ...
    public function getInstallmentStatus(): array {
        ...
        $installments = $this->installments()->with('fee')->orderBy('due_date')->get();
        ...
        $installmentAmount =
            $paymentCount > 0 ? ($this->passenger_allocated_cost - $feeAmountTotal) / $paymentCount : 0;
        ...
        foreach ($installments as $installment) {
            ...
            $amount =
                $installment->type === 'FEE' ? round($installment->fee->amount, 2) ?? 0 : round($installmentAmount, 2);

            $name = $installment->type === 'FEE' ? $installment->fee->type : null;
        ...
        if ($nextInstallment && $nextInstallment['type'] === 'FEE') {
            ...
            $nextPayment = collect($remainingInstallments)->first(fn($i) => $i['type'] === 'PAYMENT');
