...
class SeedTestUsers extends Command
{
  ...
  public function handle()
  {
    ...
    for ($i = 1; $i <= $count; $i++) {
      ...
      // Create or find the TEST tag for customer type with explicit transaction handling
      $testTag = \DB::transaction(function () {
      ...
      if (!$testTag || !$testTag->id) {
        $this->error('Failed to locate or create the TEST tag.');
        return self::FAILURE;
      }

      $customer = Customer::find($user->id);
      $customer->tags()->attach([$testTag->id]);
