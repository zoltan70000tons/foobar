...
class PassengerController extends Controller {
    ...
    public function updateSeat(Request $request) {
        ...
        $validated = $request->validate([
            ...
            'survivor_number' => [
                ...
                new UniqueSurvivorInEvent($event_id, $request->booking_id),
            ],
            ...
            'email' => ['required', 'string', 'lowercase', 'email', 'max:255', 'not_regex:/[<>{}]/'],
        ...
        if (!empty($validated['survivor_number']) && $validated['survivor_number'] !== $slot->survivor_number) {
            if (Passenger::checkSurvivorInActiveBookings($validated['survivor_number'], $booking->event_id)) {
                return response()->json(
                    [
                        'error' =>
                            'Passenger with this Survivor Number already exists in an active booking for the event.',
                    ],
                    422,
                );
            }
    ...
    */
    public function releaseSeat(Request $request) {
        ...
        if ($slotPassenger) {
            $customer = User::query()
                ->where('email', '=', $slotPassenger->email)
                ->first();
            $user = Auth::user();
            if ($customer) {
                UserLog::create([
            ...
            $results = User::with(['detail', 'survivorNumber', 'customerAddress'])
                ...
                ->where(function ($q) use ($query) {
                    $q->where('email', 'LIKE', "%{$query}%")
                        ->orWhereHas('detail', function ($q2) use ($query) {
                            $q2->where('first_name', 'ILIKE', "%{$query}%")->orWhere(
                                'last_name',
                                'ILIKE',
                                "%{$query}%",
                            );
                ...
                ->map(function ($user) use ($eventId, $request) {
                    if ($eventId) {
                        ...
                        $exists = $baseQuery->where('status', '!=', 'CANCELLED')->exists();
