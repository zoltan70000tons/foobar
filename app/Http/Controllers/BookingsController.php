  ...
  */
        ...
        try {
            return $this->withPermission([Permissions::EditBookings], function () use (
                ...
                return DB::transaction(function () use ($bookingId, $newLeadId, $request, $eventId) {
                    ...
                    $userMax =
                        $user->membershipTypes->sortByDesc('booking_number_requirement')->first()
                            ?->booking_number_requirement ?? null;
                    $isLowerTier = $userMax && $currentMaxMembership ? $userMax < $currentMaxMembership : false;
                    if ($isLowerTier) {
                        return response()->json(
                            [
                                'error' =>
                                    'The selected user has a lower membership tier than the current lead passenger.',
                            ],
                            422,
                        );
                    }
    ...
    public function switchPaymentPlan(Request $request) {
        ...
        try {
            ...
            $result = $this->bookingRepository->changePaymentPlan(
                $booking,
    ...
    public function updateBedConfig(UpdateBedConfigRequest $request, Booking $booking) {
        try {
            ...
            $this->withPermission(
                [Permissions::EditBookings],
                function ($request, $old, $booking) {
                    $booking->save();
                    return redirect()->back()->with('success', 'Bed configuration updated');
                },
                $request,
                $old,
                $booking,
            ...
            return redirect()->back()->with('error', 'Failed to update bed configuration. Please try again.');
        }
    }
}
