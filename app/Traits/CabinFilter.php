...
trait CabinFilter
{
    public function filterCabins($cabinTypeId, $cabinCategoryId, $cabinDeck = null)
    {
        ...
        $cabins = $cabinsQuery->whereDoesntHave('temporaryReservations', function ($query) {
                $query->where('expires_at', '>', Carbon::now());
            })
            ->get();

        if ($cabins->isEmpty()) {
            return ['error' => 'No cabins found or already reserved', 'status' => 404];
        }
