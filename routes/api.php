...
Route::get("/recover-account", [
  RecoverAccountController::class,
  "showRecoverForm",
])
  ->name("recover.account.form")
  ->middleware("signed:relative");

Route::post("/recover-account-verify", [
  RecoverAccountController::class,
...
Route::post("/recover-account-register", [
  RecoverAccountController::class,
  "recoverAccount",
])
...
Route::post("/email/verification-notification", [
  CustomerEmailVerificationController::class,
  "store",
...
// --- LOGOUT ---
Route::post("/logout", [CustomerLoginController::class, "destroy"])->middleware(
  ["auth:sanctum", "auth.customer"]
);

// --- CUSTOMER MIDDLEWARE AFTER LOGIN ---
Route::middleware(["auth:sanctum", "verified"])->group(function () {
  Route::get("/customer", [CustomerAuthController::class, "customer"]);
  Route::post("/reset-password-inside", [
...
Route::get("/pricing-matrix/{cabinId}", [
  PricingMatrixController::class,
  "show",
]);

// get single category
Route::get("/cabins/category/{categoryId}", [
  CabinController::class,
  "showCategory",
]);

// get cabins
Route::get("/cabins/{cabinTypeId}/{cabinCategoryId}/{cabinDeck}", [
  CabinController::class,
  "show",
]);
...
Route::middleware(["auth:sanctum", "auth.customer", "verified"])->group(
  function () {
    Route::get("/customers/{id}", [
      EditProfileController::class,
      "getAccountIntel",
    ])->where("id", "[0-9a-fA-F\-]{36}");
    Route::get("/customers/{id}/details", [
      EditProfileController::class,
      "getCustomerDetails",
    ]);
    Route::post("/customers/preferred-language", [
      EditProfileController::class,
      "updatePreferredLanguage",
    ]);
    Route::post("/customers/update-phone", [
      EditProfileController::class,
      "updatePhone",
    ]);
    Route::post("/customers/update-email", [
      EditProfileController::class,
      "updateEmail",
    ]);
    Route::post("/customers/update-password", [
      EditProfileController::class,
      "updatePassword",
    ]);
  }
