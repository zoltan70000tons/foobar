...
Route::middleware(['auth', 'electron_auth'])->group(function () {
    ...
    Route::get('/events/{id}/cabins/categories', [CabinCategoriesController::class, 'index'])->name(
        'cabins.categories',
    );
    ...
    Route::put('bookings/{booking}/bed-config', [BookingsController::class, 'updateBedConfig'])->name(
        'bookings.update-bed-config',
    );
    ...
    Route::get('/cabins/upgrade-list', [BookingsController::class, 'getCabinsToUpgradeTo'])->name(
        'cabins.upgrade-list',
    );
    ...
    Route::get('/generate-booking-pdf', [EmailController::class, 'generateBookingPDF'])->name(
        'email.generateBookingPDF',
    );
    Route::get('/generate-img', [EmailController::class, 'generateBookingIMG'])->name('email.generateBookingIMG');
    Route::get('/generate-invoice-pdf', [EmailController::class, 'generateInvoicePDF'])->name(
        'email.generateInvoicePDF',
    );

    Route::get('/potential-survivor-matches', [PotentialSurvivorMatchesController::class, 'index'])->name(
        'matches.index',
    );
    Route::get('/potential-survivor-matches/paginated', [PotentialSurvivorMatchesController::class, 'getPaginated']);
    Route::get('/potential-survivor-matches/{id}', [PotentialSurvivorMatchesController::class, 'show'])->name(
        'matches.show',
    );
    Route::delete('/potential-survivor-matches/{id}', [PotentialSurvivorMatchesController::class, 'destroy'])->name(
        'matches.destroy',
    );
    Route::put('/potential-survivor-matches/{id}/update', [PotentialSurvivorMatchesController::class, 'update'])->name(
        'matches.update',
    );
    Route::put('/potential-survivor-matches/{id}/resolve', [
        PotentialSurvivorMatchesController::class,
        'resolve',
    ])->name('matches.resolve');
    Route::post('/sync-survivors', [PotentialSurvivorMatchesController::class, 'syncSurvivors'])->name(
        'matches.run-manual-sync',
    );
...
Route::prefix('payment-transfer')->group(function () {
    Route::post('{event_id}/{booking_id}/store', [
        \App\Http\Controllers\PaymentTransferController::class,
        'store',
    ])->name('manual.payment-transfer');
